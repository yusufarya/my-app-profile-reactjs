...
function TabButton(props) {
    ...
    if (props.isActive) {
        return (
            <NavLink to={"/"} className="hover:border-b-2 border-grey-500 border-b-2 border-black rounded-lg lg:mx-3 md:mx-2 sm:mx-2 xs:mx-2 my-2 lg:px-2 md:px-2 mx-3 px-1 pt-2 inline-grid" >
                ...
                {props.children == 'Notes' && <BiSolidFileTxt className="rounded-lg lg:h-5 lg:w-5 md:h-6 md:w-6 sm:h-5 sm:w-5 h-7 w-7 text-gray-950 hover:text-gray-500" />}
                {props.children == 'Projects' && <AiOutlineRocket className="rounded-lg lg:h-6 lg:w-6.5 md:h-6.5 md:w-6 sm:h-5 sm:w-5 h-7 w-7 text-gray-950 hover:text-gray-500" />}
